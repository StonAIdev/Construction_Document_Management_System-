...
const PrepareCoverForm = ({
  ...
  const uploadMultipleLogos = async (logos, coverpage_id) => {
    for (const key in logos) {
      if (logos[key].file) putFileToS3(coverpage_id, key, logos[key].file);
    }
    return true;
  };
  ...
  const fetchLogoFromS3 = async (coverPage_id, varLogoList) => {
    for (const key in varLogoList) {
      if (varLogoList[key].name !== "")
        varLogoList[key].blob = await getLogoFromS3(key, coverPage_id);
    }
  ...
  const getlogos = (coverPage_id, logoList) => {
    fetchLogoFromS3(coverPage_id, logoList);
  };
  const ActionOnForm = () => {
    if (coverPagePhaseRef.current === "preparation") {
  ...
  const getCoverPageDetails = async (coverpage_id) => {
    ...
    try {
      ...
      getlogos(CoverPageContext.coverpage_id, res.data._source.logoList);
      ...
      CoverPageContext.coverPageOf = res.data._source.coverPageOf;
      ActionOnForm();
      return true;
